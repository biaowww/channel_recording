...
2. **选「录制目标」**：最上面的下拉框，选你要录的程序。默认停在"— 请选择要录的程序 —"，**不选不让开始**。
   - ⚠️ **两个下拉别搞混**：上面的**「录制目标」决定录谁的声音**；下面的**「投屏来源」只决定抓谁的画面**。
     要录会议就把**「录制目标」也选成会议软件**（只在「投屏来源」选了会议是不够的，那样只会抓到画面、录到别的程序的声音）。
     两者不是同一个程序时，程序会弹窗提醒你确认。
...
4. **（勾了"抓投屏"才需要）选「投屏来源」** —— 你要录屏幕的哪一块：
   ...
   - 📐 **清晰度 = 目标在屏幕上的实际大小**（抓到的就是屏幕上那么多像素）。想要 PPT 看得清，**把会议窗口最大化**再录；
     录制中界面会显示 `抓取 1920x1080`，**这个数字太小就说明窗口太小/选错了窗口**，早发现早重来。
...
- **录出来是静音的 / 录成了别的程序？** → 十有八九是**「录制目标」选错了**（它决定录谁的声音，跟「投屏来源」是两码事）。
  录制中盯一眼界面的**音量条**：一直是空的就说明目标选错或它没在出声，赶紧停下重选，别等录完。
  文件名取自「录制目标」那个程序的窗口标题，名字不对也说明目标选错了。
- **中途切换了扬声器/音频输出设备会不会录不到？** → 不会。进程回环抓的是**该进程渲染出来的音频流**，不绑定输出设备。
- **slide 一直是 0 / 一张图都没抓到？** → 十有八九是**会议窗口被最小化了**（比如你切到了会议软件的小浮窗）。最小化的窗口 Windows 根本不渲染，任何截屏方式都抓不到——**被别的窗口盖住没关系，最小化不行**。
  现在程序录制中会**自动把它还原**（不抢焦点、不弹到前面）并在界面提示"已自动还原"；看到 `⚠ 投屏窗口被最小化` 这类提示就说明它正在处理。窗口被关掉了它也会提示。
- **抓到的 PPT 很糊？** → 抓的就是它在屏幕上的实际像素。把会议窗口**最大化**再录；界面上的 `抓取 WxH` 能告诉你当前分辨率。
